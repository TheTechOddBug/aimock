...
- **Faithful reasoning capture + replay** — the recorder now preserves provider reasoning artifacts that the stream collapsers previously dropped. Anthropic `signature_delta` (the real cryptographic thinking-block `signature`) and `redacted_thinking` block `data` are captured, Bedrock Converse `reasoningContent` deltas and Cohere `thinking` content-deltas are captured into reasoning, and a recorded Anthropic `reasoningSignature` is written into the saved fixture (`TextResponse` / `ToolCallResponse` / `ContentWithToolCallsResponse`). On replay, Anthropic emits the recorded signature when one was captured and otherwise falls back to the round-trip-safe placeholder, across both streaming and non-streaming text, content+tool, and tool-only response shapes.
